...
List<ExpressCompanyInfoBean> list = (List<ExpressCompanyInfoBean>)request.getAttribute("CompanyList");
...
<p class="article_tit">快递公司列表</p>
<form name="formSearch" method="post" action="/express/ExpressCompanyInfoList.do">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_style">
    ...
    <%
    for(ExpressCompanyInfoBean bean:list){ 
    %>
    <tr>
      <td><%=bean.getCompanyId() %></td>
      <td><%=bean.getCompanyCode() %></td>
      <td><%=bean.getCompanyName() %></td>
      <td align="center"><%if(adminHelper.hasPermission(ExpressResource.EXPRESS_COMPANY, OperationConfig.UPDATE)){ %><a href="/express/ExpressCompanyInfoManage.do?companyId=<%=bean.getCompanyId() %>">修改</a><%} %></td>
    </tr>
    <%} %>
  </table>
</form>
...
<li class="page_left_btn">
  <%if(adminHelper.hasPermission(ExpressResource.EXPRESS_COMPANY, OperationConfig.CREATE)){ %><input type="button" name="button" id="button" value="新建" onclick="javascript:location.href='/express/ExpressCompanyInfoManage.do'"><%} %>
</li>
