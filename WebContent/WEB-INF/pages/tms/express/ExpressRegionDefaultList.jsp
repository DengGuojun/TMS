...
List<ExpressRegionDefaultBean> list = (List<ExpressRegionDefaultBean>)request.getAttribute("ExpressRegionList");
...
<title>默认快递地区列表</title>
<%@ include file="../../include/header.jsp" %>
</head>
<body class="article_bg">
<p class="article_tit">默认快递地区列表</p>
<form name="formSearch" method="post" action="/express/ExpressRegionDefaultList.do">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_style">
    ...
    <%for(ExpressRegionDefaultBean bean:list){
    %>
    <tr>
      <td><%=MapKit.getValueFromMap(bean.getStoreId(), storeMap) %></td>      
      <td><%=bean.getCountry() %></td>
      <td><%=bean.getProvince() %></td>
      <td><%=bean.getCity() %></td>
      <td><%=MapKit.getValueFromMap(bean.getCompanyId(), companyMap) %></td>
      <td align="center"><%if(adminHelper.hasPermission(ExpressResource.EXPRESS_REGION_DEFAULT, OperationConfig.UPDATE)){ %><a href="/express/ExpressRegionDefaultManage.do?expressId=<%=bean.getExpressId() %>">修改</a><%} %></td>
    </tr>
    <%} %>
  </table>
</form>
...
<li class="page_left_btn">
  <%if(adminHelper.hasPermission(ExpressResource.EXPRESS_REGION_DEFAULT, OperationConfig.CREATE)){ %><input type="button" name="button" id="button" value="新建" onclick="javascript:location.href='/express/ExpressRegionDefaultManage.do'"><%} %>
</li>
